...
class Members{
    ...
    public function membersByType(){

        //select all data
        $query = "SELECT s.idmembership as statusID, s.name, count(*) as `count`
    ...
    public function activeMembersByType(){
        ...
        $query = "SELECT s.idmembership as statusID, s.name, count(*) as `count`, s.multiplier
                    ,SUM(IFNULL(m.multiplier,s.multiplier))/count(*) as actmultiplier
                    ,SUM(IFNULL(m.multiplier,s.multiplier)) as contribution
                    FROM knightsb_membership.member m
                    LEFT JOIN membershipstatus s ON m.membership_idmembership = s.idmembership
